...
inputBTN.addEventListener("click", buscarPizza,localStorage1 );

function buscarPizza(e) {
  e.preventDefault();
  const pizzaID = input.values;
  if (pizzaID === "") {
    showError("Por favor ingresa un id valido para la pizza  !");
    return;
  }
  createHTML();
  input.values = "";
...
function createHTML() {
  cartList.innerHTML = "";
  if (pizzas.some((pizza) => pizza.nombre == input.values)) {
...
let zappi = pizzas.values()
localStorage.setItem("pizza",JSON.stringify (zappi))
